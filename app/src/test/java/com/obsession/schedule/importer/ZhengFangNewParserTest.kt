 ...
 * 正方教务课表页解析器的单元测试。
 *
 * 夹具来自用户真实导出的教务页面（示例大学 / 正方 jwglxt 新版），
...
class ZhengFangNewParserTest {
    ...
    @Test
    fun `课表模式会剥掉教室前面的校区名`() {
        // 课表模式的原文是「示例大学  图书馆303计算机软件开放实验室」（中间两个空格）
        val grid = (HtmlScheduleImporter.parse(readUtf8("zhengfang_grid.html"))
            as ParseOutcome.Success).courses
    ...
    fun `解析不出周次时返回 null 而不是瞎猜`() {
        assertEquals(null, WeekSpec.parse("软件工程 示例大学 图书馆303"))
    }
